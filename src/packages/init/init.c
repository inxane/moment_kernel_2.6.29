...
int load_wimax_driver();
int unload_wimax_driver();
#endif
    ...
    if (rev) {
		...
		if (x) {
            revision = strtoul(x + 2, 0, 16);
			{
		    	...
		    	snprintf(klpath, sizeof(klpath), "/tmp/s3c-keypad-rev%s.kl", rev_str);
        	}
		}
    }
}
...
#ifdef WIFI_ADDR
#define PATH_WIFI_MACADDR_INFO "/data/.mac.info"

int check_wifi_address()
{
    int rc;
    ERROR/*INFO*/("wifi.mac_addr: check_wifi_address()\n");

    // if exists, return 0, if non-exists return -1
    if (access(PATH_WIFI_MACADDR_INFO, F_OK)) {
        int rc;

        rc = load_wifi_driver();
        if (rc) {
            ERROR("wifi.mac_addr: Loading wifi-module failed!\n");
            return rc;
        }

        rc = unload_wifi_driver();
        if (rc) {
            ERROR("wifi.mac_addr Unloading wifi-module failed!\n");
            return rc;
        }

        ERROR/*INFO*/("wifi.mac_addr: .mac.info %s\n",
                        (access(PATH_WIFI_MACADDR_INFO, F_OK))?"not found!":"well created!" );
        return 0;
    }

    ERROR/*INFO*/("wifi.mac_addr: .mac.info already exist!\n");
    return 0;
}
#endif

#if defined (CONFIG_MACH_BONANZA)
int check_wimax_address()
{
    int rc;
    ERROR/*INFO*/("wimax===============\n");

        rc = load_wimax_driver();
        if (rc) {
            ERROR(" Loading wimax-module failed!\n");
            return rc;
        }

/*        rc = unload_wimax_driver();
        if (rc) {
            ERROR("wimax Unloading wimax-module failed!\n");
            return rc;
        }
*/
        return 0;
...
int main(int argc, char **argv)
{
	...
	mknod("/dev/ttygs0", S_IFCHR | 0660, (127 << 8) | 0);
	mknod("/dev/ttySMD0", S_IFCHR | 0660, (251 << 8) | 4);
#if defined (CONFIG_MACH_BONANZA)
	mknod("/dev/uwibro", S_IFCHR | 0666, (233 << 8) | 0);	//sangam
	mknod("/dev/swmxctl", S_IFCHR | 0666, (234 << 8) | 0);	//sangam
#endif
#ifdef CONFIG_MACH_INSTINCTQ
    mknod("/dev/ppp", S_IFCHR | 0666, (108 << 8) | 0);  // make node for pppd
#endif

    open_devnull_stdio();
    log_init();
    MSG("!@Init process start\n");
    ...
    //property_set("ro.build.PDA", "I570EXXJA1");  // Show version *#1234#
    snprintf(tmp, PROP_VALUE_MAX, "%d", revision);
    property_set("ro.revision", tmp);
	property_set("ro.keylayout.path", klpath);

    if(factorytest == 2) {
	    property_set("ro.secure","0");
	    property_set("ro.debugable","1");
...
#ifdef WIFI_ADDR
    check_wifi_address();  // wifi_macaddr
#endif

#if defined (CONFIG_MACH_BONANZA)
//    check_wimax_address();  // wifi_macaddr
#endif
    for(;;) {
        int nr, i, timeout = -1;
