    ...
    switch (type)
    {
       ...
       //필요에따라 사용하시기 바랍니다.
		...
		case 2: // auto
	        ...
	        s5k4ca_sensor_write(client, 0x0028, 0x7000);
	        s5k4ca_sensor_write(client, 0x002A, 0x030E);    
            s5k4ca_sensor_write(client, 0x0F12, 0x00FE); // dummy lens position
			...
			s5k4ca_sensor_write(client, 0x002A, 0x030E);    
            s5k4ca_sensor_write(client, 0x0F12, 0x00FF); // move lens to 0xFF (normal mode initial lens position)
            msleep(50);     
		...
		case 4: // macro
			...
			s5k4ca_sensor_write(client, 0x002A, 0x030E);	
			s5k4ca_sensor_write(client, 0x0F12, 0x0048);
			...
			s5k4ca_sensor_write(client, 0x002A, 0x030E);	
			s5k4ca_sensor_write(client, 0x0F12, 0x0040);  // move lens to 0x40 (macro mode initial lens position)
			msleep(50);
